...
| --------- | ------ | ----- |
| Display | :heavy_check_mark: |  - VRR works from GNOME 46 (tested on Fedora Workstation 40) <br><br> - On supported displays (laptop's built-in display supports HDR only through Proton and on mpv) HDR works system-wide on Gnome and Plasma... Chromium based browsers need `chrome://flags/#enable-experimental-web-platform-features` ON <br><br> - HDR works with Proton-GE on Heroic launcher (confirmed myself). Should probably work in steam too with the right settings <br><br> - HDR works in mpv  <br><br> - Displays colors by default are good enough. <br><br> - [Disable AMD Vari Bright](https://github.com/realKarthikNair/16-xf0xxx-linux-troubleshooting/blob/main/disable-amd-Vari-Bright.md) (THIS IS NECESSARY if you care about color accuracy on battery power) | 
| Keyboard | :heavy_check_mark: | - Keyboard backlit can be turned ON and OFF from Linux <br><br> - You can't control the RGB settings of keyboard on this model from Linux yet. [hp-omen-linux-module](https://github.com/pelrun/hp-omen-linux-module) doesn't work either. However, if you're on dual boot, (only static) RGB settings changed from Windows will persist on Linux as long as BIOS isn't soft-reset (once BIOS is soft-reset it defaults to the default multi-zone colors) <br><br> - You can't control the brightness of the keyboard from Linux either <br><br> - Surprisingly, the calculator key does launch the calculator app on Gnome <br><br> - Orange indicator on mute button doesn't work on Linux but the button itself does the mute/unmute function | 
...
| DisplayPort | :heavy_check_mark: | - DisplayPort works as expected ~(but only when booted on dGPU)~ on both hybrid and dedicated mode over USB C <br><br> - turn "High resolution mode on USB-C DP alt mode dock" ON in bios for proper bandwidth to USB C DP 1.4a ports. Without this, my ex271q had contrast issues at 2k@180 hz |
...
| blank screen no output | | - unrelated to linux but your hall sensor is probably fucked up (known issue with this model because it sits too close to a heat vent). Open laptop lid and disconnect the ribbon connector (but accept that your laptop can never automatically go back to sleep when lid is closed) or get it fixed at a repair place or an HP authorised place (I've went with the former since my unit has more like become a desktop computer atp). <br><br> - This issue is discussed well on reddit.com/r/HPOmen/
